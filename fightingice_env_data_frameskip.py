...
class FightingiceEnv_Data_Frameskip(gym.Env):
    ...
    def __init__(self, **kwargs):
        ...
        if "show_training" in kwargs.keys():
            self.show_training = kwargs["show_training"]
        ...
        self.observation_space = spaces.Box(low=0, high=1, shape=(144,))
        ...
        if self.system_name == "windows":
            ...
            if not self.show_training:
                self.java_env = subprocess.Popen(["java", "-XstartOnFirstThread", "-cp", self.start_up_str, "Main", "--port", str(self.port), "--py4j", "--fastmode",
                                            "--grey-bg", "--inverted-player", "1", "--mute", "--limithp", "400", "400", "--disable-window"], stdout=devnull, stderr=devnull)
            else:
                self.java_env = subprocess.Popen(["java", "-XstartOnFirstThread", "-cp", self.start_up_str, "Main", "--port", str(self.port), "--py4j", "--fastmode",
                                            "--grey-bg", "--inverted-player", "1", "--mute", "--limithp", "400", "400"], stdout=devnull, stderr=devnull)
